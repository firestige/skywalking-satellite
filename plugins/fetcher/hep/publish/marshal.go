...
func (h *HepMsg) String() string {
	...
	s := strings.Join([]string{`HEP packet:{`,
		fmt.Sprintf("Version:%v,", h.Version),
		fmt.Sprintf("Protocol:%v,", h.Protocol),
		fmt.Sprintf("SrcIP:%v,", h.SrcIP),
		fmt.Sprintf("DstIP:%v,", h.DstIP),
		fmt.Sprintf("SrcPort:%v,", h.SrcPort),
		fmt.Sprintf("DstPort:%v,", h.DstPort),
