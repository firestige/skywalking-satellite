...
func cacheCID(srcIP []byte, rtcpIP []byte, rtcpPort []byte, callID []byte) {
	...
	log.Logger.Debugf("Add to cidCache key=%q, value=%q", key, callID)

	cidCache.Set(key, callID, cidCacheTime)
	if !bytes.Equal(rtcpIP, srcIP) {
		key = append(append(append(buffer[:0], srcIP...), ' '), rtcpPort...)
		log.Logger.WithField("source", "sdp").Debugf("Add to cidCache key=%q, value=%q", key, callID)
		cidCache.Set(key, callID, cidCacheTime)
	}
}
...
sdpLoop:
	for posLine = 0; posLine < len(content); posLine = posLineEnd + 1 {
		...
		if len(line) < 2 || line[1] != '=' {
			...
			if !multipart {
				log.Logger.WithField("source", "sdp").Debugf("Fishy sdp line %q. callID=%q", line, callID)
			}
			continue sdpLoop
		}
		...
		case 'c':
			...
			if !bytes.HasPrefix(line, []byte("c=IN IP")) || len(line) < 16 {
				log.Logger.WithField("source", "sdp").Debugf("Fishy c= line %q. callID=%q", line, callID)
				continue sdpLoop
			}
			// Extract IP.
			ip := line[9:]
		...
		case 'm':
			...
			if sep < 4 { // Port should be above 1000
				log.Logger.WithField("source", "sdp").Debugf("Fishy m=audio line %q. callID=%q", line, callID)
				continue sdpLoop
			}
			// Extract RTP port.
		...
		case 'a':
			...
			if sep < 0 {
				...
				if !bytes.HasPrefix(line[7+sep+1:], []byte("IN IP")) {
					log.Logger.WithField("source", "sdp").Debugf("Fishy a=rtcp line %q. callID=%q", line, callID)
					continue sdpLoop
				}
				// Extract port.
...
func correlateRTCP(srcIP net.IP, srcPort uint16, dstIP net.IP, dstPort uint16, payload []byte) ([]byte, []byte) {
	...
	if info != "" {
		log.Logger.WithField("source", "rtcp").Debugf("Parsing rtcp returned info. ssrc=%x, srcIP=%v, srcPort=%v, dstIP=%v, dstPort=%v, info=%q",
	...
	if err == nil && rtcpKey != nil {
		log.Logger.WithField("source", "rtcp").Debugf("Found key=%q value=%q in rtcpCache for srcIP=%v, srcPort=%v, dstIP=%v, dstPort=%v",
	...
	if err == nil {
		log.Logger.WithField("source", "rtcp").Debugf("Found key=%q value=%q in cidCache for srcIP=%v, srcPort=%v, dstIP=%v, dstPort=%v",
	...
	log.Logger.WithField("source", "rtcp").Debugf("No correlationID for srcIP=%v, srcPort=%v, dstIP=%v, dstPort=%v",
		srcIP, srcPort, dstIP, dstPort)
...
func correlateLOG(payload []byte) (byte, []byte) {
	...
	if posID := bytes.Index(payload, []byte("ID=«")); posID > 0 {
		...
		if posRestID := bytes.IndexRune(restID, '»'); posRestID >= 5 {
			...
			log.Logger.WithField("source", "log").Debugf("No end or fishy Call-ID in '%s'", restID)
			return 0, nil
		}
	...
	if len(callID) > 1 && len(callID) < 80 {
		log.Logger.WithField("source", "log").Debugf("Found CallID: %s in Logline: '%s'", callID, payload)
		return 100, callID
	}
	return 0, nil
}
