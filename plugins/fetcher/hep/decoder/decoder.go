...
import (
	...
	"github.com/apache/skywalking-satellite/plugins/fetcher/hep/decoder/internal"
	"github.com/apache/skywalking-satellite/plugins/fetcher/hep/ip4defrag"
	"github.com/apache/skywalking-satellite/plugins/fetcher/hep/ip6defrag"
	"github.com/apache/skywalking-satellite/plugins/fetcher/hep/protos"
...
const (
	ProtoTypeUnknown  byte = 0x00 // 未知类型
	ProtoTypeSIP      byte = 0x01 // SIP 信令
	ProtoTypeRTP      byte = 0x04 // RTP 媒体
	ProtoTypeRTCPJson byte = 0x05 // RTCP 控制 JSON 格式
	ProtoTypeRTCPRaw  byte = 0x40 // RTCP 控制 原始格式
	ProtoTypeSTUN     byte = 0x41 // STUN 协议
	ProtoTypeDTLS     byte = 0x42 // DTLS 协议
	ProtoTypeSIPLOG   byte = 0x64 // SIP 日志
)
...
func (d *Decoder) Process(data []byte, ci *gopacket.CaptureInfo) {
	...
	for _, layer := range d.decodedLayers {
		switch layer {
		case layers.LayerTypeIPv4:
			...
			if d.ip4.Flags&layers.IPv4DontFragment != 0 || (d.ip4.Flags&layers.IPv4MoreFragments == 0 && d.ip4.FragOffset == 0) {
				d.processTransport(&d.decodedLayers, &d.eth, &d.ip4, &d.ip6, &d.udp, &d.tcp, &d.sctp, d.ip4.NetworkFlow(), ci, 0x02, uint8(d.ip4.Protocol), d.ip4.SrcIP, d.ip4.DstIP)
				break
			}
			...
			if err != nil {
				log.Logger.Warnf("%v, srcIP: %s, dstIP: %s\n\n", err, d.ip4.SrcIP, d.ip4.DstIP)
				return
			} else if ip4New == nil {
			...
			if ip4New.Length == ip4Len {
				...
				log.Logger.WithField("source", "defrag").Infof("%d byte fragment layer: %s with payload:\n%s\n%d byte re-assembled payload:\n%s\n\n",
					ip4Len, d.decodedLayers, d.ip4.Payload, ip4New.Length, ip4New.Payload,
				)

				switch ip4New.Protocol {
				case layers.IPProtocolUDP:
					d.parserUDP.DecodeLayers(ip4New.Payload, &d.decodedLayers)
				case layers.IPProtocolTCP:
					d.parserTCP.DecodeLayers(ip4New.Payload, &d.decodedLayers)
				default:
					log.Logger.Warn("unsupported IPv4 fragment layer")
					return
				}
				d.processTransport(&d.decodedLayers, &d.eth, &d.ip4, &d.ip6, &d.udp, &d.tcp, &d.sctp, ip4New.NetworkFlow(), ci, 0x02, uint8(ip4New.Protocol), ip4New.SrcIP, ip4New.DstIP)
			}

		case layers.LayerTypeIPv6:
			atomic.AddUint64(&d.ip6Count, 1)
			if d.ip6.NextHeader != layers.IPProtocolIPv6Fragment {
				d.processTransport(&d.decodedLayers, &d.eth, &d.ip4, &d.ip6, &d.udp, &d.tcp, &d.sctp, d.ip6.NetworkFlow(), ci, 0x0a, uint8(d.ip6.NextHeader), d.ip6.SrcIP, d.ip6.DstIP)
				break
			}
			...
			if ip6frag := packet.Layer(layers.LayerTypeIPv6Fragment).(*layers.IPv6Fragment); ip6frag != nil {
				...
				switch ip6New.NextHeader {
				case layers.IPProtocolUDP:
					d.parserUDP.DecodeLayers(ip6New.Payload, &d.decodedLayers)
				case layers.IPProtocolTCP:
					d.parserTCP.DecodeLayers(ip6New.Payload, &d.decodedLayers)
				default:
					log.Logger.Warn("unsupported IPv6 fragment layer")
					return
				}
				d.processTransport(&d.decodedLayers, &d.eth, &d.ip4, &d.ip6, &d.udp, &d.tcp, &d.sctp, ip6New.NetworkFlow(), ci, 0x0a, uint8(ip6New.NextHeader), ip6New.SrcIP, ip6New.DstIP)
			}
		}
	}
}
...
func startLineResponse(val string) int {
	...
	log.Logger.Warnf("startLineResponse missed response text val = %s", val)
	return indexStart
}
...
func (d *Decoder) addCachePayload(cachePayload *CachePayload) (result bool) {
	...
	if err != nil {
		log.Logger.Errorf("addCachePayload err = %v, payload = [%s]", err, string(cachePayload.Payload))

		log.Logger.Errorf("addCachePayload err = %v src_ip = %v, src_port = %d, dst_ip = %v, "+
...
func (d *Decoder) checkTransport(srcIP net.IP, srcPort uint16, dstIP net.IP, dstPort uint16, tcp *layers.TCP) (result bool, payLoadList *list.List) {
	...
	timeDelay := time.Since(d.lastStatTime).Milliseconds()
	if timeDelay > 60*1000 {
		log.Logger.Infof("checkTransport expired_count = %d, miss_count = %d, entry_count = %d, "+
	...
	if err == nil && len(nextPayloadBytes) > 0 {
		if !d.cachePayload.Del(nextCacheBuffer.Bytes()) {
			log.Logger.Error("checkTransport del next cache failed")
		}
		err = json.Unmarshal(nextPayloadBytes, &nextCachePayload)
		if err != nil {
			log.Logger.Errorf("checkTransport unmarshal next cache err = %v", err)
		}
	}
	...
	if err == nil && len(lastPayloadBytes) > 0 {
		if !d.cachePayload.Del(lastCacheBuffer.Bytes()) {
			log.Logger.Error("checkTransport del last cache failed")
		}
		err = json.Unmarshal(lastPayloadBytes, &lastCachePayload)
		if err != nil {
			log.Logger.Errorf("checkTransport unmarshal last cache err = %v", err)
	...
	if len(lastCachePayload.Payload) > 0 {
		if seqNumber == lastCachePayload.NextSeqNumber {
			...
			if lastCachePayload.SeqNumber != seqNumber ||
				...
				log.Logger.Errorf("checkTransport retransmission payload src_ip = %v, src_port = %d, dst_ip = %v, "+
				...
				log.Logger.Errorf("checkTransport retransmission last = [%s], current = [%s]", string(lastCachePayload.Payload), string(payload))
			}
		}
	...
	if len(nextCachePayload.Payload) > 0 {
		if (seqNumber + uint32(payloadLen)) == nextCachePayload.SeqNumber {
			...
			if nextCachePayload.SeqNumber != seqNumber ||
				...
				log.Logger.Errorf("checkTransport retransmission next = [%s], current = [%s]", string(nextCachePayload.Payload), string(payload))
			}
		}
	...
	if len(lastCachePayload.Payload) > 0 && len(nextCachePayload.Payload) > 0 {
		log.Logger.Infof("checkTransport combine last and next payload src_ip = %v, src_port = %d, dst_ip = %v, "+
		...
		log.Logger.Infof("checkTransport combine last and next last = [%s], current = [%s], next = [%s]",
	...
	for {
		...
		processCount++
		if processCount > 30 {
			log.Logger.Errorf("checkTransport invalid process src_ip = %v, src_port = %d, dst_ip = %v, "+
				"dst_port = %d, ack_number = %d, seq_number = %d, next_seq_number = %d, frame_count = %d",
				srcIP, srcPort, dstIP, dstPort, ackNumber, processCachePayload.SeqNumber,
				processCachePayload.NextSeqNumber, processCachePayload.FrameCount)
			log.Logger.Errorf("checkTransport invalid process full_payload = [%s], current_payload = [%s]",
				byteBuffer.String(), string(currentPayLoad))
			break
		}
		...
		for indexHeader := 0; indexHeader < len(headerLines); indexHeader++ {
			...
			if strings.Contains(headerLine, "SIP/2.0 200 OK") {
				log.Logger.Errorf("checkTransport invalid header line src_ip = %v, src_port = %d, dst_ip = %v, "+
					"dst_port = %d, ack_number = %d, seq_number = %d, next_seq_number = %d, frame_count = %d",
					srcIP, srcPort, dstIP, dstPort, ackNumber, processCachePayload.SeqNumber,
					processCachePayload.NextSeqNumber, processCachePayload.FrameCount)
				log.Logger.Errorf("checkTransport invalid header line full_payload = [%s], current_payload = [%s]",
					byteBuffer.String(), string(currentPayLoad))
			}
		...
		if err != nil || len(callID) <= 0 {
			log.Logger.Errorf("checkTransport no call_id err = %s, src_ip = %v, src_port = %d, dst_ip = %v, "+
				"dst_port = %d, ack_number = %d, seq_number = %d, next_seq_number = %d, frame_count = %d",
				err.Error(), srcIP, srcPort, dstIP, dstPort, ackNumber, processCachePayload.SeqNumber,
				processCachePayload.NextSeqNumber, processCachePayload.FrameCount)
			log.Logger.Errorf("checkTransport no call_id full_payload = [%s], current_payload = [%s]",
				byteBuffer.String(), string(currentPayLoad))
			currentPayLoad = content
			continue
		}
		...
		if contentLength < 0 {
			log.Logger.Errorf("checkTransport invalid content_length src_ip = %v, src_port = %d, dst_ip = %v, "+
				"dst_port = %d, ack_number = %d, seq_number = %d, next_seq_number = %d, frame_count = %d",
				srcIP, srcPort, dstIP, dstPort, ackNumber, processCachePayload.SeqNumber,
				processCachePayload.NextSeqNumber, processCachePayload.FrameCount)
			log.Logger.Errorf("checkTransport invalid content_length full_payload = [%s], current_payload = [%s]",
				byteBuffer.String(), string(currentPayLoad))
			currentPayLoad = content
			continue
		}
		...
		if contentLength < currentContentLen {
			log.Logger.Infof("checkTransport multi payload content_length = %d, current_content_len = %d, call_id = %s, src_ip = %v, src_port = %d, dst_ip = %v, "+
				"dst_port = %d, ack_number = %d, seq_number = %d, next_seq_number = %d, frame_count = %d",
			...
			log.Logger.Infof("checkTransport multi payload content_length full_payload = [%s], current_payload = [%s]",
				byteBuffer.String(), string(currentPayLoad))
		}
	}
...
func (d *Decoder) processTransport(foundLayerTypes *[]gopacket.LayerType, eth *layers.Ethernet, ip4 *layers.IPv4, ip6 *layers.IPv6, udp *layers.UDP, tcp *layers.TCP, sctp *layers.SCTP, flow gopacket.Flow, ci *gopacket.CaptureInfo, IPVersion, IPProtocol uint8, sIP, dIP net.IP) {
	if d.cfg.DiscardIP != "" {
		for _, v := range d.filterIP {
			if dIP.String() == v {
				log.Logger.Infof("discarding destination IP: %s", dIP.String())
				return
			}
			if sIP.String() == v {
				log.Logger.Infof("discarding source IP: %s", sIP.String())
				return
			}
		}
	}
	...
	for _, layerType := range *foundLayerTypes {
		...
		case layers.LayerTypeUDP:
			if len(udp.Payload) < 16 {
				log.Logger.Warnf("received too small %d byte UDP packet with payload %v", len(udp.Payload), udp.Payload)
				return
			}
			...
			log.Logger.Infof("payload - UDP: %s", string(pkt.Payload))
		...
		case layers.LayerTypeTCP:
			...
			log.Logger.Infof("payload - TCP: %v", pkt)
			...
			if d.cfg.SipAssembly {
				...
				for elem := payloadList.Front(); elem != nil; elem = elem.Next() {
					pkt.CID = extractCID(pkt.SrcIP, pkt.SrcPort, pkt.DstIP, pkt.DstPort, elem.Value.([]byte))
				}
			} else {
				pkt.Payload = buildFullPayload(foundLayerTypes, IPVersion, eth, ip4, ip6, tcp, udp, sctp)
				pkt.CID = extractCID(pkt.SrcIP, pkt.SrcPort, pkt.DstIP, pkt.DstPort, pkt.Payload)
		...
		case layers.LayerTypeSCTP:
			...
			log.Logger.Infof("payload - SCTP: %v", pkt)
...
func buildFullPayload(foundLayerTypes *[]gopacket.LayerType, IPVersion uint8, eth *layers.Ethernet, ip4 *layers.IPv4, ip6 *layers.IPv6, tcp *layers.TCP, udp *layers.UDP, sctp *layers.SCTP) []byte {
	var fullPayload []byte
	fullPayload = append(fullPayload, eth.Contents...)
	switch IPVersion {
	case 0x02:
		fullPayload = append(fullPayload, ip4.Contents...)
	case 0x0a:
		fullPayload = append(fullPayload, ip6.Contents...)
	}
	for _, layerType := range *foundLayerTypes {
		switch layerType {
		case layers.LayerTypeTCP:
			fullPayload = append(fullPayload, tcp.Contents...)
			fullPayload = append(fullPayload, tcp.Payload...)
		case layers.LayerTypeUDP:
			fullPayload = append(fullPayload, udp.Contents...)
			fullPayload = append(fullPayload, udp.Payload...)
		case layers.LayerTypeSCTP:
			fullPayload = append(fullPayload, sctp.Contents...)
			fullPayload = append(fullPayload, sctp.Payload...)
		}
	}
	return fullPayload
}
