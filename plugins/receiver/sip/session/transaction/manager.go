...
func (m *TransactionManager) CreateTransaction(msg types.SipMessage) (*TransactionContext, error) {
	if req, ok := msg.(types.SipRequest); ok {
		...
		case types.MethodInvite:
			...
			if ua == types.UAClient {
				return NewTransaction(req, &InviteCallingState{}), nil
			}
			if ua == types.UAServer {
				return NewTransaction(req, &InviteProceedingState{}), nil
			}
		default:
			return NewTransaction(req, &NonInviteTryingState{}), nil
		}
	}
	return nil, fmt.Errorf("unsupported SIP message type %T for transaction creation", msg)
}
